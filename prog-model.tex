...
use of hard-coded imperative methods. This limits the range of potential automatic optimization or optimizations derived from static analysis.  Based on the
challenges outlined in the previous section, we present an alternative, declarative
...
declarative language for expressing distributed programs as unordered sets
of rules~\cite{alvaro:cidr11}. Bloom rules fully specify program semantics and
mask details associated with program
...
declarative language. Log data and metadata are represented by two
persistent collections possessing abstract mappings to physical storage (Figure~\ref{fig:pm} (c) Lines 1-2). Such a transformation
...
refer to \cite{watkins:ucsc-soe-16-12} for a full program listing.} are expressible using only a few Bloom code snippets.
...
the experience of designing and writing an interface such as CORFU in 
